...
| <a name="input_db_password"></a> [db\_password](#input\_db\_password) | MySQL password for the admin user (generated if not provided) | `string` | `""` | no |
| <a name="input_db_user"></a> [db\_user](#input\_db\_user) | MySQL admin user name (default is service name) | `string` | `""` | no |
...
| <a name="input_grants"></a> [grants](#input\_grants) | List of { grant: "[<grant>, <grant>, ...]", db: "db" }.<br/>Normal grants plus `ALL_APP` for all RDS allowed grants that an app should need<br/>(can be limited to a single database). `ALL` is not the normal MySQL `ALL` but<br/>is all the grants RDS allows. | <pre>list(object({<br/>    grant : list(string)<br/>    db : string<br/>  }))</pre> | <pre>[<br/>  {<br/>    "db": "*",<br/>    "grant": [<br/>      "ALL_APP"<br/>    ]<br/>  }<br/>]</pre> | no |
| <a name="input_id_length_limit"></a> [id\_length\_limit](#input\_id\_length\_limit) | Limit `id` to this many characters (minimum 6).<br/>Set to `0` for unlimited length.<br/>Set to `null` for keep the existing setting, which defaults to `0`.<br/>Does not affect `id_full`. | `number` | `null` | no |
| <a name="input_kms_key_id"></a> [kms\_key\_id](#input\_kms\_key\_id) | KMS key ID, ARN, or alias to use for encrypting MySQL database | `string` | `"alias/aws/rds"` | no |
...
| <a name="output_password_ssm_key"></a> [password\_ssm\_key](#output\_password\_ssm\_key) | SSM key under which user password is stored |
